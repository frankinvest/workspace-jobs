...
### 💼 2026-09-24 持仓变更（Frank 指令，当天上线）

- 卖出 **华夏银行** 1700 股 @ **6.23** → 22700 → **21000 股**；摊薄成本 **6.788 → 6.833**（卖价低于成本，剩余成本被抬高）
- 买入 **海南橡胶**（`601118`）1500 股 @ **6.68**（新增；code 用腾讯行情源核过名字）
- **追加一笔（同日）**：再卖 **华夏银行 500 股 @ 6.23** → 21000 → **20500 股**，摊薄成本 6.833 → **6.848**
  （Frank 原话「再更新下持仓，又以6.23卖出500股」——按同价同标的＝华夏银行处理）
- 落地：`data/trades.json` +2 笔 → `tools/recalc_holdings.py`（摊薄）→ `data/stock_holdings.json` 21 → **22 条**
  → build 212 页 → Contents API 推送 → 线上实测 `600015 → 21000/6.833`、`601118 → 1500/6.68` ✅

### ✅ 2026-09-23 / 09-24 早报验证新流水线（发布时自动本地化）生效

- 09-23：9 张图，09-24：10 张图，**均在发布时自动下载到 `public/images/JJC-<date>/`**，md 里红圈外链 = 0
- 线上逐张实测 **全部 200**（09-23 9/9、09-24 10/10）→ `finance_breakfast.py::step_images` 的改动确认可用，
  「文章过 7 天图全黑」这个故障从流程上不会再出现
