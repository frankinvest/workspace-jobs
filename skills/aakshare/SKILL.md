...
**问题**：VPN 开启时，akshare 默认使用的 eastmoney 数据源可能被拦截，导致 `ProxyError`。

**已内置解决方案**：本 skill 已包含腾讯行情 API 绕过脚本，无需 eastmoney：

```python
from tencent_stock import get_quote, get_index

# 获取个股实时行情（推荐，绕过 eastmoney 拦截）
df = get_quote(['sh600000', 'sz000001'])
...
**何时用哪个**：
- `get_quote()` → 实时个股行情（腾讯API，不过 eastmoney）
- `akshare` 原生接口 → 历史K线、财务数据（非实时，VPN 下可能也可用）
